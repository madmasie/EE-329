...
/* Instructions:
(a) LED Counter - Connect 4 LEDs to GPIO pins PC0, PC1, PC2, and PC3. (series LED and 560 Ω R).
Use data on the LED datasheet to calculate the expected current from a VOH output then measure
the actual GPIO output voltage, LED voltage, and LED ON current. Write code in main() that repeatedly 
...
Further leverage ExecutionTime.c, specifically the TestFunction() function, to add instruction execution 
timing to your main() routine. Code it so that the execution timing starts after 3 iterations through 
the count to 15 on the LEDs then repeats continuously.Set up an oscilloscope or logic analyzer to capture
...
and PC1 is toggled before and after the arithmetic instruction inside the function. Change var_type and 
TestFunction(num) to test the combinations listed in Table A1(b). Run the code iteratively with the prescribed
